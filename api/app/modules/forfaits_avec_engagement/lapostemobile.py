...
class Lapostemobile(BaseScraper):
    def __init__(self):
        ...
        logging.info("Initialized La Poste Mobile Scraper Avec Engagement.")
    ...
    def process_data(self):
        ...
        for name, is_5g, price, price_addon in zip(extracted_names, extracted_5g, plan_prices, extracted_5g_Add):
            self.plans.append({'name': name, 'is_5g': is_5g, 'price': price, 'price_addon': price_addon, 'annee': 2})
    ...
    def insert_data(self):
        ...
        for plan in self.plans:
            ...
            forfait_id = self.db_operations.insert_into_forfaits(self.operator_data['OperateurID'], limite, unite, 0, 0, 0, 1, plan['annee'])
            self.db_operations.insert_into_tarifs(forfait_id, plan['price'], date_enregistrement)
            logging.info(f"Inserted plan {plan['name']} with price {price_float:.2f} with is5G False")
            if plan['is_5g']:
