...
from modules.azure_sql_db import load_environment_variables, get_database_connection, fetch_all_operator_plan_details, check_date_exists

app = Flask(__name__)

# Assuming you have a function to fetch all operator plan details
def create_and_save_excel(cursor):
...
def run_tasks():
    ...
    #Check if current date already exist in db
    if not check_date_exists(cursor):
        ...
        excel_file = create_and_save_excel(cursor)
